...
public class FacturaDAOPagination implements GenericDAO<Factura>{

    private static final String SQLSELECTALL = "SELECT * FROM facturas LIMIT ?";
    ...
    private static final String SQLSELECTPAGE = "SELECT * FROM facturas ORDER BY id LIMIT ? OFFSET ?";
    private static final String SQLVENDEDOR = "SELECT * FROM facturas WHERE vendedor = ? ORDER BY id LIMIT ? ";
    private static final String SQLCLIENTE = "SELECT * FROM facturas WHERE cliente = ? ORDER BY id LIMIT ?";
    private static final String SQLVENDEDORPAGE = "SELECT * FROM facturas WHERE vendedor = ? ORDER BY id LIMIT ?";
    private static final String SQLCLIENTEPAGE = "SELECT * FROM facturas WHERE cliente = ? ORDER BY id LIMIT ? OFFSET ?";
    ...
    private final PreparedStatement pstSelectPage;
    private final PreparedStatement pstVendedorPage;
    private final PreparedStatement pstClientePage;
    private int pageSize = 100;


    public FacturaDAOPagination() throws SQLException {
        ...
        pstSelectPage = con.prepareStatement(SQLSELECTPAGE);
        pstVendedorPage = con.prepareStatement(SQLVENDEDORPAGE);
        pstClientePage = con.prepareStatement(SQLCLIENTEPAGE);
    }
    ...
    public long getNumPages() throws SQLException {
    ...
    public List<Factura> findAll(long pagina) throws SQLException {
        List<Factura> facturaList = new ArrayList<Factura>();
        long offset = (pagina - 1) * pagina;
        pstSelectPage.setInt(1, pageSize);
        pstSelectPage.setLong(2,offset);
        ResultSet rs = pstSelectPage.executeQuery();
        while (rs.next()) {
    ...
    public List<Factura> findByCliente(int id, long pagina) throws SQLException {
        List<Factura> facturaList = new ArrayList<Factura>();
        long offset = (pagina - 1) * pagina;
        pstClientePage.setInt(1,id);
        pstClientePage.setInt(2, pageSize);
        pstClientePage.setLong(3,offset);
        ResultSet rs = pstClientePage.executeQuery();
        while (rs.next()) {
            facturaList.add(build(rs.getInt("id"), rs.getDate("fecha").toLocalDate(), rs.getInt("cliente"), rs.getInt("vendedor"), rs.getString("formapago")));
        }
        return facturaList;
    }
    public List<Factura> findByVendedor(int id, long pagina)throws SQLException{
        List<Factura> facturaList = new ArrayList<Factura>();
        long offset = (pagina - 1) * pagina;
        pstVendedorPage.setInt(1,id);
        pstVendedorPage.setInt(2, pageSize);
        pstVendedorPage.setLong(3,offset);
        ResultSet rs = pstVendedorPage.executeQuery();
        while (rs.next()) {
            facturaList.add(build(rs.getInt("id"), rs.getDate("fecha").toLocalDate(), rs.getInt("cliente"), rs.getInt("vendedor"), rs.getString("formapago")));
        }
        return facturaList;
    ...
    public List<Factura> findAll() throws SQLException {
        List<Factura> facturaList = new ArrayList<Factura>();
        pstSelectAll.setInt(1,pageSize);
